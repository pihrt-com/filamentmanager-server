...
SMTP is configured in Administration > Settings. Notification types and thresholds are configured for each user. For automatic evaluation and delivery, run `php bin/notifications.php` every five minutes when CLI cron is available. On shared hosting that supports HTTPS cron only, copy the secret web-cron URL generated under Settings and schedule that exact URL instead. SMTP passwords are encrypted with the installation application key and are intentionally excluded from portable backups; enter the password again after restoring onto a clean installation.
