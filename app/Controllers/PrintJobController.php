...
final class PrintJobController
{
    ...
    public function repeat(Request $request,string $id): void
    {
        Csrf::verify($request);$user=$this->app->auth()->requireRole('admin','manager','operator');$count=(new PrintJobService($this->app))->repeat($user['workspace_id'],$id,$user['id']);(new NotificationService($this->app))->evaluateWorkspace($user['workspace_id']);(new AuditService($this->app))->record('print_job.repeated','print_job',$id,null,['deduction_count'=>$count]);Session::flash('success',View::t('print_job_repeated',['count'=>$count]));Response::redirect($request->basePath().'/print-jobs/'.$id);
    }
