...
final class PrinterController
{
    ...
    public function form(Request $request, ?string $id = null): void
    {
        ...
        $spools = $this->app->db()->fetchAll("SELECT s.id,s.current_net_weight_g,s.status,s.notes,s.batch_number,m.material_type,m.color_name,m.commercial_name,mf.name manufacturer_name,l.name location_name,ps.printer_id loaded_printer_id FROM spools s JOIN materials m ON m.id=s.material_id LEFT JOIN manufacturers mf ON mf.id=m.manufacturer_id LEFT JOIN locations l ON l.id=s.location_id LEFT JOIN printer_slots ps ON ps.loaded_spool_id=s.id AND ps.deleted_at IS NULL WHERE s.workspace_id=? AND s.deleted_at IS NULL AND (s.status='in_stock' OR ps.printer_id=?) ORDER BY mf.name,m.material_type,m.color_name,m.commercial_name,s.notes,s.id", [$user['workspace_id'], $id ?? '']);
