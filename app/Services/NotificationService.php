...
final class NotificationService
{
    ...
    public function processQueue(int $limit=20,?string $workspaceId=null): array
    {
        $params=[];$workspaceSql='';if($workspaceId!==null){$workspaceSql=' AND workspace_id=?';$params[]=$workspaceId;}$rows=$this->app->db()->fetchAll("SELECT * FROM mail_queue WHERE ((status IN ('queued','failed') AND next_attempt_at<=UTC_TIMESTAMP(6)) OR (status='sending' AND locked_at<UTC_TIMESTAMP(6)-INTERVAL 10 MINUTE)) AND attempts<5".$workspaceSql.' ORDER BY id LIMIT '.max(1,min(100,$limit)),$params);$sent=0;$failed=0;foreach($rows as $row){try{$config=$this->mailConfig($row['workspace_id']);if(($config['smtp_enabled']??'0')!=='1')continue;$claimed=$this->app->db()->execute("UPDATE mail_queue SET status='sending',locked_at=UTC_TIMESTAMP(6),attempts=attempts+1 WHERE id=? AND ((status IN ('queued','failed') AND next_attempt_at<=UTC_TIMESTAMP(6)) OR (status='sending' AND locked_at<UTC_TIMESTAMP(6)-INTERVAL 10 MINUTE))",[$row['id']])->rowCount();if($claimed!==1)continue;(new SmtpMailer())->send($config,$row['recipient'],$row['subject'],$row['body']);$this->app->db()->execute("UPDATE mail_queue SET status='sent',sent_at=UTC_TIMESTAMP(6),locked_at=NULL,last_error=NULL WHERE id=?",[$row['id']]);$sent++;}catch(\Throwable $error){$delay=min(3600,300*((int)$row['attempts']+1));$next=gmdate('Y-m-d H:i:s',time()+$delay);$this->app->db()->execute("UPDATE mail_queue SET status='failed',locked_at=NULL,last_error=?,next_attempt_at=? WHERE id=?",[mb_substr($error->getMessage(),0,500),$next,$row['id']]);$failed++;}}return ['sent'=>$sent,'failed'=>$failed];
    }
