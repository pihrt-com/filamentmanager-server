...
An update needs temporary free disk space for the archive, extracted package, and backup. If an update fails, keep the generated pre-update backup and inspect `storage/logs/app.log`. Application-file rollback packaging and cryptographic release signatures are planned before the first stable public release; until then, only releases published by the repository owner should be installed.
