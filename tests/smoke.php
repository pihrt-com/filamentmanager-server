...
$bgcode=pack('VVv',0x45444347,1,1).$bgcodeBlock(3,"filament_type=PLA;PETG\nextruder_colour=#FF0000;#0000FF\n").$bgcodeBlock(4,"filament used [g]=12.5;3.25\nestimated printing time (normal mode)=1h 2m\n",1).$bgcodeBlock(2,"printer_settings_id=MK4 test\n");
$bgcodePath=tempnam(sys_get_temp_dir(),'fm-bgcode-');if($bgcodePath===false||file_put_contents($bgcodePath,$bgcode)===false)throw new RuntimeException('Cannot create BGcode test fixture.');try{$binaryParsed=(new FilamentManager\Services\GcodeParser())->parse($bgcodePath);}finally{@unlink($bgcodePath);}if(count($binaryParsed['consumptions'])!==2||abs((float)$binaryParsed['totalWeightG']-15.75)>0.001||$binaryParsed['consumptions'][0]['materialType']!=='PLA'||$binaryParsed['consumptions'][1]['colorHex']!=='#0000FF')throw new RuntimeException('Prusa BGcode parsing failed.');
...
if(!str_contains($postProcessor,'SLIC3R_PP_OUTPUT_NAME')||!str_contains($postProcessor,'output_file_name'))throw new RuntimeException('PrusaSlicer helper does not preserve the original output filename.');
